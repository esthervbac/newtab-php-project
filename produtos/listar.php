...
<div class="container-fluid">
    ...
    <h2 class="text-center pb-3">Listagem de Produtos Cadastrados</h2>
    <br>
    <br>
        ...
        <tbody>
            <?php if ($linhas) : ?>
                <?php foreach ($linhas as $linha) :
                    $valorunitario = $linha[2];
                    $valorUnitarioFormatado = number_format($linha[2] / 100, 2, ",", ".");

                    $dataFormatada = date_format(date_create($linha[4]), 'd-m-Y H:i:s');
                ?>
                    <tr>
                        ...
                        <td scope="col">
                            ...
                            <a class='btn btn-sm btn-danger' data-bs-toggle='modal' data-bs-target="#id<?= $linha[0] ?>">
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="id<?= $linha[0] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="exampleModalLabel">Deseja deletar este produto?</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                                        </div>
                                        <div class="modal-body">
                                            Uma vez deletado não será possível recuperar o produto.
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-success" data-bs-dismiss="modal">Cancelar</button>
                                            <a href="deletar.php?id=<?= $linha[0] ?>" class="btn 
                                            btn-danger">Deletar</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
